...
try:
    ...
    logger.info(f"📊 Available databases: {[db for db in dbs if db not in SYSTEM_DBS]}")
...
async def handle_ping(request):
    print(f"✅ Ping received  at test :: {datetime.now(timezone.utc).isoformat()}")
...
async def main():
    ...
    # Start polling
    logger.info("🚀 Bot starting (robust version)...")
